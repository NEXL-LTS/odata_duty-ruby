...
- **MCP server** — tools-only over JSON-RPC; reads inferred as `list_/get_/count_<Set>` tools, writes as `create_/update_/delete_<Set>`, no resources; read tools advertise only the `odata_*` query options their set's hooks can serve, and `instructions` always carries a generated description of the supported dialect — `doc/using_mcp.md`, `doc/mcp_crash_course.md`.
