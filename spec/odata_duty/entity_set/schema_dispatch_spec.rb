...
# Class DSL only; the builder DSL is a sibling task.
DispatchContext = Struct.new(:marker)

class DispatchTestEntity < OdataDuty::EntityType
  property_ref 'id', String, computed: false
  property 'name', String
end

class DispatchUsersSet < OdataDuty::EntitySet
  entity_type DispatchTestEntity
...
class ConfirmedDeleteSet < OdataDuty::EntitySet
  entity_type DispatchTestEntity
...
class DispatchTestSchema < OdataDuty::Schema
  base_url 'http://localhost:3000/api'
  entity_sets [DispatchUsersSet, ConfirmedDeleteSet]
end

RSpec.describe OdataDuty::Schema, 'class-DSL request-dispatch entry points' do
  subject(:schema) { DispatchTestSchema }

  let(:context) { DispatchContext.new('ctx') }
  ...
  describe '.execute routes the same URL family to distinct results' do
    it 'routes the bare set name to the collection, passing the caller context through' do
      response = Oj.load(schema.execute('DispatchUsers', context: context))
    ...
    it 'routes a keyed URL to the matching individual, passing the caller context through' do
      response = Oj.load(schema.execute("DispatchUsers('2')", context: context))
    ...
    it 'routes the $count suffix to the count, passing the caller context through' do
      expect(schema.execute('DispatchUsers/$count', context: context)).to eq(2)
    end
    ...
    it 'raises ResourceNotFoundError for a missing individual' do
      expect do
        schema.execute("DispatchUsers('999')", context: context)
      end.to raise_error(OdataDuty::ResourceNotFoundError)
    end

    it 'propagates UnknownPropertyError for an unknown $select property' do
      expect do
        schema.execute('DispatchUsers', context: context, query_options: { '$select' => 'nope' })
      end.to raise_error(OdataDuty::UnknownPropertyError)
    end
  ...
  describe '.create returns the created entity' do
    it 'returns the created payload, passing body and caller context through' do
      json = schema.create('DispatchUsers', context: context,
    ...
    it 'propagates InvalidType for a body value of the wrong type' do
      expect do
        schema.create('DispatchUsers', context: context, query_options: { 'name' => 1 })
  ...
  describe '.update partial-merges and returns the updated entity' do
    it 'returns the updated payload, passing body and caller context through' do
      json = schema.update("DispatchUsers('1')", context: context,
                                                 query_options: { 'name' => 'Alice Updated' })
      expect(Oj.load(json)).to include('id' => '1', 'name' => 'Alice Updated-ctx')
    end
    ...
    it 'raises ResourceNotFoundError for a missing individual' do
      expect do
        schema.update("DispatchUsers('999')", context: context, query_options: {})
      end.to raise_error(OdataDuty::ResourceNotFoundError)
    end
  end
  ...
  describe '.delete removes and returns no entity payload' do
    it 'removes the entity for a known key, observing the caller context' do
      result = schema.delete("DispatchUsers('1')", context: context, query_options: {})
      expect(Oj.load(result)).not_to include('id')
    end

    it 'succeeds when query_options defaults to an empty hash' do
      expect do
        schema.delete("DispatchUsers('2')", context: context)
      end.not_to raise_error
    end

    it 'raises ResourceNotFoundError for a missing individual' do
      expect do
        schema.delete("DispatchUsers('999')", context: context, query_options: {})
      end.to raise_error(OdataDuty::ResourceNotFoundError)
    end

    it 'threads query options to the resolver, which deletes only when confirm is present' do
      result = schema.delete("ConfirmedDelete('1')", context: context,
                                                     query_options: { 'confirm' => 'true' })
    ...
    it 'raises ResourceNotFoundError when the confirm option is absent' do
      expect do
        schema.delete("ConfirmedDelete('1')", context: context, query_options: {})
      end.to raise_error(OdataDuty::ResourceNotFoundError)
    end
  end
end
